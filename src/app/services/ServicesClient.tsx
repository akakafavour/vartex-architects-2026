...
function ServicesIndex({ onOpenQuiz }: { onOpenQuiz: () => void }) {
    return <div className="flex flex-col"><section className="service-reveal border-b border-neutral-100 px-8 pt-2 pb-12 dark:border-white/5 lg:px-24 lg:pt-12 lg:pb-36"><span className="font-mono text-[11px] tracking-[0.4em] text-neutral-400">SERVICES</span><h1 className="mt-7 max-w-3xl text-6xl font-black uppercase leading-[0.86] tracking-tighter text-primary dark:text-white lg:text-[8rem]">Every line<br />serves<br />a purpose.</h1><p className="mt-6 max-w-xl text-lg leading-relaxed text-primary/65 dark:text-white/65 lg:mt-10 lg:text-xl">Vartex works across two specialized fields: architecture and interior design. In both, we bring the same discipline: careful planning and precise execution.</p></section>
        <section className="grid grid-cols-1 lg:grid-cols-2">{([{ id: "architecture", title: "Architectural Design", description: "We design structures from the inside out, beginning with site context, solar orientation, and structural integrity.", image: "/images/services/architectural-design.jpeg", icon: HomeIcon }, { id: "interior", title: "Interior Design", description: "We complete the space down to the last detail. From proportions to finishes to custom joinery.", image: "/images/services/interior-design.webp", icon: Sofa }] as const).map((service) => { const Icon = service.icon; return <Link key={service.id} href={`/services/${service.id}`} className="service-reveal service-card group border-b border-r border-neutral-100 bg-white p-8 text-left transition-colors hover:bg-neutral-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary dark:border-white/5 dark:bg-background-dark dark:hover:bg-neutral-900 lg:p-16"><div className="service-card-image-stage relative mb-8 aspect-[16/9] overflow-hidden bg-[var(--background)]"><Image src={service.image} alt={`${service.title} service`} fill priority sizes="(max-width: 1024px) 100vw, 50vw" className="service-card-image object-cover lg:transition-transform lg:duration-500 lg:ease-out lg:group-hover:scale-110" /><div className="service-card-image-reveal pointer-events-none absolute inset-0 bg-[var(--background)]" /></div><div className="flex items-start justify-between gap-6"><div><h2 className="mt-4 text-3xl font-black uppercase leading-none tracking-tight text-primary dark:text-white lg:text-5xl">{service.title}</h2><p className="mt-6 max-w-md text-base leading-relaxed text-primary/65 dark:text-white/65 text-layer transition-colors duration-500">{service.description}</p></div><Icon className="icon-layer mt-1 h-8 w-8 shrink-0 text-primary/25 transition-colors group-hover:text-primary dark:text-white/25 dark:group-hover:text-white duration-500" aria-hidden="true" /></div><span className="mt-10 inline-flex items-center gap-3 font-mono text-[11px] font-bold tracking-[0.2em] text-primary dark:text-white">EXPLORE THIS SERVICE <ArrowUpRight className="h-4 w-4" /></span></Link>; })}</section>
...
function ServicesDetail({ data, setPage }: { data: ServiceData; setPage: (p: "index" | "architecture" | "interior") => void }) {
    ...
    return <div className="flex flex-col overflow-x-clip"><div className="service-reveal flex items-center gap-3 border-b border-neutral-100 px-8 py-6 font-mono text-[11px] tracking-[0.15em] text-neutral-400 dark:border-white/5 lg:px-24"><Link href="/services" className="hover:text-primary dark:hover:text-white">SERVICES</Link><span>/</span><span className="text-primary dark:text-white">{data.label}</span></div><section className="service-reveal grid gap-12 border-b border-neutral-100 px-8 pt-2 pb-12 dark:border-white/5 lg:grid-cols-12 lg:px-24 lg:pt-12 lg:pb-32"><div className="lg:col-span-8"><span className="font-mono text-[11px] tracking-[0.35em] text-neutral-400">SERVICE DIRECTORY</span><h1 className="mt-5 text-[clamp(2.35rem,10.4vw,7rem)] font-black uppercase leading-[0.88] tracking-tighter text-primary dark:text-white lg:mt-7 lg:text-[7rem]">{data.headline.map((line) => <span key={line} className="block">{line}</span>)}</h1><p className="mt-6 max-w-2xl text-xl leading-relaxed text-primary/70 dark:text-white/70 lg:mt-10 lg:text-2xl">{data.hero}</p></div><div className="flex flex-col gap-3 lg:col-span-4 lg:pt-8"><button onClick={() => setShowGuide(true)} className="bg-primary py-5 font-mono text-[11px] font-bold tracking-[0.2em] text-white hover:bg-black dark:bg-white dark:text-black dark:hover:bg-neutral-200">RECEIVE SERVICE GUIDE</button><Link href="/contact" className="border border-neutral-200 py-5 text-center font-mono text-[11px] font-bold tracking-[0.2em] text-primary hover:bg-neutral-50 dark:border-white/10 dark:text-white dark:hover:bg-white/5">START A PROJECT</Link></div></section><div className="service-reveal service-image-stage relative h-[42vh] min-h-[280px] w-full overflow-hidden bg-white dark:bg-background-dark"><div className="service-detail-media group absolute inset-0"><Image src={data.image} alt={data.imageAlt} fill priority sizes="100vw" className="service-detail-image object-cover transition-transform duration-500 ease-out lg:group-hover:scale-110" /><div className="pointer-events-none absolute inset-0 dark:bg-black/20" /></div></div><section className="service-reveal border-b border-neutral-100 bg-neutral-50 px-8 py-20 dark:border-white/5 dark:bg-neutral-900/10 lg:px-24 lg:py-28"><p className="service-approach-text max-w-4xl text-2xl font-light leading-snug text-primary/80 dark:text-white/80 lg:text-4xl">{data.approach}</p></section><TierComparison data={data} />{data.buildSection && <section className="service-reveal grid gap-12 border-b border-neutral-100 bg-neutral-50 px-8 py-20 dark:border-white/5 dark:bg-neutral-900/10 lg:grid-cols-12 lg:px-24 lg:py-28"><div className="lg:col-span-5"><h2 className="text-4xl font-black uppercase leading-none tracking-tighter text-primary dark:text-white">THE BUILD.</h2><p className="mt-4 text-lg font-medium text-primary/70 dark:text-white/70">Construction Management</p></div><div className="lg:col-span-7"><p className="max-w-xl text-base leading-relaxed text-primary/65 dark:text-white/65">Already have your construction drawings ready? We can step in as lead consultant to manage the project and make sure construction stays true to the design.</p><ul className="mt-8 grid gap-4 sm:grid-cols-2">{["QS coordination and bill of quantities", "Contractor tendering and bid review", "Scheduled on-site verification inspections", "Material mockup approvals", "Payment certificate issuance", "Snag list and final handover packages"].map((item) => <li key={item} className="flex gap-3 text-sm"><span className="mt-3 h-px w-2 shrink-0 bg-current opacity-50" />{item}</li>)}</ul></div></section>}<section className="service-reveal max-lg:hidden border-b border-neutral-100 px-8 py-20 dark:border-white/5 lg:px-24 lg:py-28"><span className="font-mono text-[11px] tracking-[0.3em] text-neutral-400">HOW WE WORK</span><div className="mt-12 grid gap-10 md:grid-cols-2 lg:grid-cols-4">{data.how.map(([num, title, desc]) => <article key={num} className="border-t border-neutral-200 pt-6 dark:border-white/10"><span className="font-mono text-[11px] tracking-[0.2em] text-neutral-400">{num}</span><h3 className="mt-5 text-xl font-bold uppercase leading-tight text-primary dark:text-white">{title}</h3><p className="mt-4 text-base leading-relaxed text-primary/65 dark:text-white/65">{desc}</p></article>)}</div></section><GridCTA className="service-reveal px-8 py-20 lg:px-24 lg:py-24"><div className="flex flex-col gap-10 lg:flex-row lg:items-end lg:justify-between"><h2 className="text-6xl font-black uppercase leading-none tracking-tighter lg:text-[8rem]">LET&apos;S BUILD.</h2><div className="flex w-full flex-col gap-3 sm:flex-row lg:w-auto"><button onClick={() => setShowGuide(true)} className="bg-white px-8 py-5 font-mono text-[11px] font-bold tracking-[0.2em] text-primary hover:bg-neutral-100">RECEIVE SERVICE GUIDE</button><Link href="/contact" className="border border-white/50 px-8 py-5 text-center font-mono text-[11px] font-bold tracking-[0.2em] text-white hover:bg-white/10">START A PROJECT</Link></div></div></GridCTA>{showGuide && <GuideModal service={data.id} onClose={() => setShowGuide(false)} />}</div>;
}
    ...
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            ...
            gsap.utils.toArray<HTMLElement>(".service-card-image-stage").forEach((stage) => {
                const overlay = stage.querySelector<HTMLElement>(".service-card-image-reveal");
                if (!overlay) return;
                // Reveal via an overlay wipe only. The image itself is never
                // translated/scaled by GSAP, so hover zoom classes and touch
                // interaction cannot conflict with the reveal transform.
